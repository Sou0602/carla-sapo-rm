...
try:
  ...
  #  traffic_manager.set_synchronous_mode(True)
    ...
    mbpo = MBPO(policy, value_fun,cost_fun,simulator,model_name=model_name,eta = eta , uncertainty = uncertainty ,
                max_constraint_val = max_constraint_val, trajectory_len = trajectory_len, deltat = deltat)
    print(f'Training policy {model_name} on {env_name} environment...\n')

    actions = mbpo.train(200)

    plt.plot(np.asarray([i.numpy()[-2]for i in mbpo.simulator.traj[200].observations]))
    plt.savefig('traj.jpg')
    plt.close()
    import pandas as pd
    pd.Series(mbpo.violation).to_csv('mbpo_violation.csv')
    pd.Series(mbpo.mean_rewards).to_csv('mbpo_mean_reward.csv')

    #mbpo.policy(torch.tensor([[-90,12.5,-90,0,30]]).to(device)).log_prob(torch.tensor([0]).to(device))

    plt.figure(figsize=(10,7))
    plt.plot(mbpo.mean_rewards,label='MBPO')
    plt.legend()
    ...
    plt.figure(figsize=(10,7))
    plt.plot(cpo.violation,label='MBPO')
    plt.legend()
    plt.xlabel('The Number of Epoch',fontsize=20)
    plt.ylabel('Episode Constraint Violation Distance',fontsize=20)
    plt.grid()
