...
class World(object):
    ...
    def restart(self, args):
        ...
        if self.player is not None:
            ...
            self.destroy()
            self.player = self.world.try_spawn_actor(veh_blueprint, spawn_point)
        ...
        while self.player is None:
            ...
            spawn_point =spawn_points[430]
            self.player = self.world.try_spawn_actor(veh_blueprint, spawn_point)
            self.modify_vehicle_physics(self.player)

        # Spawn the player1
        if self.player1 is not None:
            spawn_point = self.player1.get_transform()
            spawn_point.location.z += 2.0
            spawn_point.rotation.roll = 0.0
            spawn_point.rotation.pitch = 0.0
            self.destroy()
            self.player1 = self.world.try_spawn_actor(veh_blueprint, spawn_point)
...
class RampMergeEnv(object):
    ...
    def __init__(self, ego, host, world ,uncertainty=0.01, expected_a=10, deltat: float = 0.01,
    ...
    def step(self, action):
        ...
        [evx,evy,evz] = self.world.player.get_velocity()
        e_speed = np.square.sum([evx,evy])
        e_noise = np.random.normal(0, self.sigma)
        ex,ey,ez = world.player.get_transform().location

        #Action is the control input:
        self.long_control = action ## include bounds
...
def game_loop(args):
    ...
    try:
        ...
        ramp_merge_env = RampMergeEnv(world.player,world.player1,world)
